...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 사이드바는 항상 표시됨 (닫기 버튼으로만 숨김)
    if (chatbotSidebar) {
        chatbotSidebar.style.display = 'flex';
    }
    ...
    if (chatbotClose) {
        chatbotClose.addEventListener('click', function() {
            if (chatbotSidebar) {
                chatbotSidebar.style.display = 'none';
            }
        });
    }
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
        const chatbotSidebar = document.getElementById('chatbotSidebar');
        if (chatbotSidebar && chatbotSidebar.style.display !== 'none') {
            chatbotSidebar.style.display = 'none';
        }
    }
});

// 사이드바는 항상 표시되므로 말풍선 기능 제거
